...
public class UserService {
	...
	public boolean blockUser(Integer idUser) throws Exception {
		...
		if (this.userRepository.findById(idUser).isPresent()) {
			...
			ArrayList<PasswordResetTokenEntity> passwordTokens = passwordTokenRepository
					.findAllByUserEntity(userExisting);
	...
	public UserDTO getLoggedUser() {
		try {
			...
			if (e.getMessage() != null) {
				log.info("An error occurred on getting logged user: ", e.getMessage());
			}
			return null;
		}
	}
